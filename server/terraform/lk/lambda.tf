...
resource "aws_s3_bucket" "lambda_bucket" {
  bucket_prefix = "ilussobsaapi-lambda-bucket"
  force_destroy = true
}

resource "aws_iam_role" "main_exec" {
  name = "ilussobsaapi-main-exec"
...
resource "aws_iam_policy" "bucketDynamoAndInvoke" {
  name        = "ilussobsaapi-bucketDynamoAndInvoke"
  path = "/ilussobsaapi/bucketDynamoAndInvoke/"
  description = "Access to the ilussobsaapi bucket, dynamo, and invoke"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_lambda_function" "main" {
  function_name = "ilussobsaapi-main"
  ...
  runtime = "java17"
  handler = "com.lightningkite.rock.template.AwsHandler"
...
resource "aws_cloudwatch_log_group" "main" {
  name = "ilussobsaapi-main-log"
  retention_in_days = 30
}

resource "local_sensitive_file" "settings_raw" {
  content = jsonencode({
    serveApp = var.serveApp
    oauthApple = var.oauthApple
    cache = {
        url = "dynamodb://${var.deployment_location}/ilussobsaapi"
    }
    ...
    database = {
      url = "mongodb+srv://ilussobsaapidatabase-main:${random_password.database.result}@${replace(mongodbatlas_serverless_instance.database.connection_strings_standard_srv, "mongodb+srv://", "")}/default?retryWrites=true&w=majority"
    }
    geocoding = var.geocoding
    sms = var.sms
    logging = var.logging
    files = {
    ...
    oauthGoogle = var.oauthGoogle
    oauthMicrosoft = var.oauthMicrosoft
    email = {
