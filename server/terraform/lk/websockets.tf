...
resource "aws_apigatewayv2_stage" "ws" {
  ...
  name = "ilussobsaapi-gateway-stage"
  auto_deploy = true
...
resource "aws_cloudwatch_log_group" "ws_api" {
  name = "ilussobsaapi-ws-gateway-log"
...
resource "aws_iam_policy" "api_gateway_ws" {
  name        = "ilussobsaapi-api_gateway_ws"
  path = "/ilussobsaapi/api_gateway_ws/"
  description = "Access to the ilussobsaapi_api_gateway_ws management"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
