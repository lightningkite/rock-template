...
resource "aws_apigatewayv2_stage" "ws" {
  ...
  name = "hammerprice-gateway-stage"
  auto_deploy = true
...
resource "aws_cloudwatch_log_group" "ws_api" {
  name = "hammerprice-ws-gateway-log"
...
resource "aws_iam_policy" "api_gateway_ws" {
  name        = "hammerprice-api_gateway_ws"
  path = "/hammerprice/api_gateway_ws/"
  description = "Access to the hammerprice_api_gateway_ws management"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
