...
resource "aws_s3_bucket" "lambda_bucket" {
  bucket_prefix = "hammerprice-lambda-bucket"
  force_destroy = true
}

resource "aws_iam_role" "main_exec" {
  name = "hammerprice-main-exec"
...
resource "aws_iam_policy" "bucketDynamoAndInvoke" {
  name        = "hammerprice-bucketDynamoAndInvoke"
  path = "/hammerprice/bucketDynamoAndInvoke/"
  description = "Access to the hammerprice bucket, dynamo, and invoke"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_lambda_function" "main" {
  function_name = "hammerprice-main"
...
resource "local_sensitive_file" "settings_raw" {
  content = jsonencode({
    ...
    cache = {
        url = "dynamodb://${var.deployment_location}/hammerprice"
    }
    ...
    database = {
        url = "postgresql://master:${random_password.database.result}@${aws_rds_cluster.database.endpoint}/hammerpricedatabase"
    }
